...
public class IndexTools {
    ...
    public void createIndex() throws InvalidTokenOffsetsException {
        String indexPath = "e://lucene//luceneIndex"; // 建立索引文件的目录
        ...
        try {
            ...
            document.add(new TextField("id", "标题:ID", Store.YES));
            document.add(new TextField("title", "内容：人本 经典款帆布鞋 韩版百搭布鞋休闲男鞋 舒适 防滑", Store.YES));
            indexWriter.addDocument(document);
            Document document1 = new Document();
            document1.add(new TextField("id", "标题:终点", Store.YES));
            document1.add(new TextField("title", "内容：布鞋", Store.YES));
...
//        higherIndex(analyzer, isearcher, query, topDocs);
    ...
    public void higherIndex(Analyzer analyzer, IndexSearcher isearcher, Query query, TopDocs topDocs)
        ...
        TopScoreDocCollector results = TopScoreDocCollector.create(topDocs.totalHits);
        ...
        // 关键字高亮显示的html标签，需要导入lucene-highlighter-5.3.1.jar
        ...
        for (int i = 0; i < docs.length; i++) {
            ...
            String content = highlighter.getBestFragment(tokenStream, doc.get("title"));
